...
void top_k_per_row_prefill_bf16(const torch::Tensor& logits,
                                const torch::Tensor& rowStarts,
  ...
  if (numColumns <= kBF16Max2PassLen) {
    ...
    topKPerRowPrefillBF16Register
        <<<numRows, kBF16BlockSize, smemBytes, stream>>>(
            scores, rowStarts.data_ptr<int>(), rowEnds.data_ptr<int>(),
            indices.data_ptr<int>(), static_cast<int>(stride0),
...
#else
  TORCH_CHECK(false, "BF16 top-k prefill is not supported on ROCm");
#endif
}
...
from .cache_utils import _encode_e4m3fn
...
def compress_norm_rope_store_triton(
    ...
    On SM80 (Ampere A100) the FP8 paths route to the ``_sm80`` variants,
    which replace the ``tl.float8e4nv`` cast (unsupported on SM80) with the
    software ``_encode_e4m3fn`` helper. The MXFP4 path needs no SM80 variant
    because it never uses ``tl.float8e4nv``.
    """
    is_sm80 = current_platform.is_device_capability((8, 0))
    if head_dim == 512:
        kernel = (
        ...
        # MXFP4 uses software _fp32x2_to_fp4x2; no SM80 variant needed.

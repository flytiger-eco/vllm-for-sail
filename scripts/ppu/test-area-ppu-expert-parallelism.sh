...
# ------------------------------------------------------------------------------
# 调用方：.github/workflows/test-area-ppu-expert-parallelism.yml（容器内，cwd = /workspace）。
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/expert_parallelism.yaml
# 的迁移快照（见下方 EP_SINGLE_ARGS / EP_MULTI_ARGS，调整用例直接改这里）。
# 本 area 的两个用例（EPLB 算法 / 权重重排）纯张量运算，不下载模型。
#
# 环境变量：
#   TEST_MODE   all(默认) | single | multi   — 对应 Aone 两个 ptg-ai-test job
#
# 机制移植自 aone_ci/scripts/test_area_ppu_expert_parallelism.sh（该文件
# AUTO-GENERATED 不可手改，故在此复刻）：
#   - single: 单进程单 step，限制可见 1 卡（对齐 Aone 1-PPU pod 语义）
#     eplb_algorithm → test_eplb_algo.py（纯 EPLB rebalance 算法，单卡）
#   - multi:  单进程（测试内部经 eplb_utils.distributed_run 起 world_size=2
#     子进程用 2 卡；world_size=4 用例在 <4 卡时自动 skip）
#     eplb_execution → test_eplb_execute.py（多卡 EPLB 权重重排）
#   - junit:  每 step 落 xml，EXIT trap 合并到 test-results/test.xml，
#     pytest 崩溃时也要补 error case（不能让 CI 信号失真）
#
# 注：eplb_utils.py 模块级 `mp.set_start_method("spawn", force=True)`，
# distributed_run 用 spawn 起子进程，无 CUDA fork-bug 隐患（reference #11）。
# ==============================================================================
...
TMP_JUNIT="/tmp/ppu-expert-parallelism-junit"
mkdir -p "${RESULTS_DIR}" "${TMP_JUNIT}"

# ------------------------------------------------------------------------------
# [tests] 用例选集（快照自 aone_ci/ppu_extras/expert_parallelism.yaml single/multi 段）
# ------------------------------------------------------------------------------
# 对位上游 .buildkite/test_areas/expert_parallelism.yaml（3 steps），PPU scope：
#   ✅ test_eplb_algo.py    — 纯 EPLB rebalance 算法，单卡
#   ✅ test_eplb_execute.py — 多卡 EPLB 权重重排（world_size=2/4）
#   ❌ test_eplb_spec_decode.py — CI 镜像 lm_eval 版本过旧，simple_evaluate
#      (model_args=dict) → AttributeError: 'dict' has no attribute 'strip'；
#      需升级镜像 lm_eval 后解锁（故 multi 段仅跑 test_eplb_execute.py）
#   ❌ test_eplb_fused_moe_layer_dep_nvfp4.py — NVFP4 quant（SM100+/Blackwell only）
#   ❌ test_elastic_ep.py   — 上游标 device: b200, optional: true；需 Ray +
#      外网 + DeepSeek-V2-Lite-Chat
#   ❌ test_eplb_fused_moe_layer.py — 不在上游 yaml 内
# single = Aone "expert-parallelism single" job（1-PPU pod）
EP_SINGLE_ARGS=(
  tests/distributed/test_eplb_algo.py
)

# multi = Aone "expert-parallelism multi" job（2-PPU pod）；test_eplb_execute.py
# 内部经 distributed_run 起 world_size=2 子进程（占 2 卡），world_size=4 用例
# 在 <4 卡时自动 pytest.skip
EP_MULTI_ARGS=(
...
# ------------------------------------------------------------------------------
# 本 area 两个用例（test_eplb_algo.py / test_eplb_execute.py）均为纯张量
# 算法/权重重排，不加载任何真实模型，故 MODEL_MAP 留空。保留此段骨架以便
# 后续放开需要模型的用例（如 test_elastic_ep.py）时补 HF id → NAS 路径映射。
echo "========== [setup] HF cache symlinks (/nas_aisw models) =========="
python3 - <<'PYEOF'
import os

MODEL_MAP = {
    # 本 area 无模型依赖（纯 EPLB 算法 / 权重重排）。
}
...
root = ET.Element("testsuites", name="vLLM PPU Expert Parallelism (GHA)")
for label in LABELS:
...
lines = ["### Expert Parallelism Test (PPU)", "",
...
if [ "${MODE}" = "single" ]; then
  # Aone single 是 1-PPU pod：限 1 卡对齐语义
  CUDA_VISIBLE_DEVICES=0 _run_step "eplb_algorithm" 1 "${EP_SINGLE_ARGS[@]}"
elif [ "${MODE}" = "multi" ]; then
  # multi：不限卡，test_eplb_execute.py 内部 distributed_run 起 world_size=2 占 2 卡
  _run_step "eplb_execution" 1 "${EP_MULTI_ARGS[@]}"
else  # all
  CUDA_VISIBLE_DEVICES=0 _run_step "eplb_algorithm" 1 "${EP_SINGLE_ARGS[@]}"
  _run_step "eplb_execution" 1 "${EP_MULTI_ARGS[@]}"
fi

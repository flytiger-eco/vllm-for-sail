...
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/quantization.yaml 的
# 迁移快照（见下方 QUANT_CORE_ARGS / QUANT_MODELS_ARGS / QUANT_BNB_ARGS，调整
# 用例直接改这里）。模型走 /nas_aisw 预置卷（docker -v /nas_aisw:/nas_aisw + HF_HUB_CACHE）。
#
# 环境变量：
#   TEST_MODE   all(默认) | single   — 本 area 为 single-only（无 multi step）
#
# 机制移植自 aone_ci/scripts/test_area_ppu_quantization.sh（该文件
# AUTO-GENERATED 不可手改，故在此复刻）：
#   - single: 3 个独立单进程 step，各限 1 卡（quantization 无 multi_gpu_test 用例）：
#       quantization_core        tests/quantization/（GPTQ/AutoRound/INT8）
#       quantization_models      tests/models/quantization/（GPTQ-Marlin/AWQ）
#       quantization_bitsandbytes tests/models/quantization/test_bitsandbytes.py（BNB，-k 选红区已 stage 模型）
#   - 大量文件因 PPU SM8.0 < SM8.9（fp8）/ 缺包（compressed_tensors/quark/torchao）
#     / 模型未 stage 被 ignore，逐文件原因见下方注释（源自 ppu_extras 审计）。

...
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/misc.yaml 的
# 迁移快照（见下方 MISC_SINGLE_* 数组，调整用例直接改这里）。
...
#
# 机制移植自 aone_ci/scripts/test_area_ppu_misc.sh（该文件 AUTO-GENERATED 不可
# 手改，故在此复刻）：
#   - single: 5 个独立单进程 step（v1_core_gpu / v1_inference / v1_spec_decode /
#     v1_connectors_metrics / async_engine_utils），逐个限制可见 1 卡执行

#!/bin/bash
# ==============================================================================
# scripts/ppu/test-area-ppu-cuda.sh — PPU CUDA 测试执行（GitHub Actions）
# ------------------------------------------------------------------------------
# 调用方：.github/workflows/test-area-ppu-cuda.yml（K8s worker Pod 内，
#         cwd = /workspace/source）。
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/cuda.yaml 的
# 迁移快照（见下方 CUDA_PLATFORM_ARGS / CUDA_CUDAGRAPH_ARGS，调整用例直接改这里）。
# 模型走 /nas_aisw 预置卷（HF_HUB_CACHE + hub cache symlink）。
#
# 环境变量：
#   TEST_MODE   all(默认) | single | multi   — cuda area 仅 single 段有用例
#
# 机制移植自 aone_ci/scripts/test_area_ppu_cuda.sh（该文件 AUTO-GENERATED 不可
# 手改，故在此复刻）：
#   - single: 两个独立 step，各单进程单卡
#       cuda_platform → tests/cuda/ CUDA 上下文 + 平台初始化测试
#       cudagraph     → tests/v1/cudagraph/ dispatch + mode 测试
#   - multi:  cuda area 无 multi 段（上游 .buildkite/test_areas/cuda.yaml 亦无）
...
TMP_JUNIT="/tmp/ppu-cuda-junit"
mkdir -p "${RESULTS_DIR}" "${TMP_JUNIT}"

# ------------------------------------------------------------------------------
# [tests] 用例选集（快照自 aone_ci/ppu_extras/cuda.yaml single 段）
# ------------------------------------------------------------------------------
# ppu_extras 审计结论：
...
#   非 pytest）。仅列这两个文件，与上游 buildkite commands 一致
#   （tests/cuda/ 下另有 test_cuda_compatibility_path.py，上游选集未纳入）。
CUDA_PLATFORM_ARGS=(
...
# （见 test-area-ppu-basic-correctness.sh 同段注释与 reference.md 踩坑清单
# 第 10 条；Aone 侧从不设它且全绿，DEC-0013 当时明确决定不引入）。
...
# torch.compile 类测试 BackendCompilerFailed）——cudagraph mode 用例走
# VLLM_COMPILE 编译路径，依赖此段。
...
MODEL_MAP = {
    # test_cudagraph_mode.py::test_cudagraph_compilation_combo /
    # test_cudagraph_correctness_with_combos 的唯一 base（函数级 LLM(...) 加载）。
    # scripts/ppu/model_alises/*.json 未收录非 GGUF 版；命中 Aone 别名
    # aone_ci/scripts/ppu_model_aliases.json（/ppusw → /nas_aisw 同构）。
...
root = ET.Element("testsuites", name="vLLM PPU CUDA (GHA)")
for label in LABELS:
...
# cuda area 仅 single 段有用例（上游 buildkite 无 multi/TP step）。
# multi 模式无 step 可跑：给出提示直接退出，避免 CI 假绿误判。
if [ "${MODE}" = "multi" ]; then
  echo "[run] cuda area has no multi-mode steps configured — nothing to run" >&2
else  # single | all
  _run_step "cuda_platform" 1 "${CUDA_PLATFORM_ARGS[@]}"
  _run_step "cudagraph" 1 "${CUDA_CUDAGRAPH_ARGS[@]}"
fi

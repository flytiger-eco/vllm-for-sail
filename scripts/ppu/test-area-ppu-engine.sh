...
#   快照时只取 test_arg_utils.py 而非整个 tests/engine/ 目录，原因：
...
ENGINE_SINGLE_E2E_ARGS=(
  ...
  # gemma-3n-E2B-it 模型较大，首版不覆盖
  --ignore=tests/v1/e2e/general/test_kv_sharing_fast_prefill.py
  # Qwen3-Next-80B-A3B-Instruct-FP8，PPU 单卡跑不了 80B
  --ignore=tests/v1/e2e/general/test_mamba_prefix_cache.py
  # Qwen3-Embedding-0.6B embedding model，首版 defer

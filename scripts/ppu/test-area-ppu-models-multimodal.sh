...
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/models_multimodal.yaml
# single 段的迁移快照（见下方 MM_GENERATION_ARGS / MM_PROCESSING_ARGS，调整用例直接改这里）。
...
#
# 机制移植自 aone_ci/scripts/test_area_ppu_models_multimodal.sh（该文件
# AUTO-GENERATED 不可手改，故在此复刻），single 段两个 step：
#   - mm_generation_dedicated: tests/models/multimodal -m core_model，排除
#     test_common 及一批离线/依赖不满足的文件（见 [tests]）；实际命中
#     core_model 的仅 test_nemotron_parse.py + pooling/test_llava_next.py
#   - mm_processing: processing/test_tensor_schema.py（多模态 processor tensor
#     schema 验证；import test_common 触发模块级视频下载，见 [assets]）
...
# ------------------------------------------------------------------------------
# [deps] area 特有依赖：opencv（cv2）。tests/models/multimodal/generation/
# test_common.py 在 module-level 构造 VLM_TEST_SETTINGS 时急切调用
# custom_inputs.video_with_metadata_glm4_1v() → VideoAsset.np_ndarrays →
# vllm/assets/video.py:video_to_ndarrays() `import cv2`；mm_processing 段的
# test_tensor_schema.py `from .test_common import ...` 会触发该 import 链，
# 缺 cv2 则 collection 阶段 ModuleNotFoundError。镜像预装则跳过——不碰镜像
# 已有栈；缺失才从 flytiger PyPI 补（纯 wheel，临时安装安全；镜像 rebake
# 预装后本段可删）。
# 备注：上游每 step `pip install Mantis`，但 PPU 仅 -m core_model，不 collect
# mantis 用例，mantis_patch_hf_runner 为 lazy import，故不装 Mantis
# （对齐 aone_ci/ppu_extras/models_multimodal.yaml 审计小结）。
# ------------------------------------------------------------------------------

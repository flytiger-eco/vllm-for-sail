#!/bin/bash
# ==============================================================================
# scripts/ppu/test-area-ppu-entrypoints-llm.sh — PPU Entrypoints LLM 测试执行（GitHub Actions）
# ------------------------------------------------------------------------------
# 调用方：.github/workflows/test-area-ppu-entrypoints-llm.yml（容器内，
# cwd = /workspace）。
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/entrypoints_llm.yaml
# 的迁移快照（见下方 EP_LLM_SINGLE_*_ARGS / EP_LLM_MULTI_RPC_ARGS，调整用例
# 直接改这里）。模型走 /nas_aisw 预置卷（docker -v /nas_aisw:/nas_aisw +
# HF_HUB_CACHE）。
#
# 环境变量：
...
# ------------------------------------------------------------------------------
# [tests] 用例选集（快照自 aone_ci/ppu_extras/entrypoints_llm.yaml）
# ------------------------------------------------------------------------------
# 本 area 对位上游 entrypoints.yaml 的 "Entrypoints Integration (LLM)" step
# 子集（单 file 拆细）；其他 entrypoints step 归 entrypoints area。
#
# Step 1: test_chat — 跳过 vision(Phi-3.5) + thinking(Qwen3-0.6B) fixture
# 相关 tests（原 yaml 注释）；两 fixture 均为函数级、仅服务被排除用例，
# -k 排除后不实例化，故无需为其建 symlink
EP_LLM_SINGLE_CHAT_ARGS=(
  tests/entrypoints/llm/test_chat.py
  -k
  'not test_chat_multi_image and not test_chat_extra_kwargs'
)
...
MODEL_MAP = {
    # test_chat.py text_llm / llm_for_failure_test fixture；清单命中 Llama/v3.2
    "meta-llama/Llama-3.2-1B-Instruct":
        "/nas_aisw/datasets/checkpoints/LLM/Llama/v3.2/Llama-3.2-1B-Instruct",
    # test_collective_rpc.py（load_format=dummy，仍需 config/tokenizer）；
    # 与 basic-correctness 同源（runner 已 ls 确认存在）
    "hmellor/tiny-random-LlamaForCausalLM":
        "/nas_aisw/datasets/checkpoints/LLM/tiny/v1.0/tiny-random-LlamaForCausalLM",
    # test_gpu_utilization.py（3 实例 @ 0.3 gpu_memory）；清单命中 misc/v1.0
    "facebook/opt-125m":
        "/nas_aisw/datasets/checkpoints/LLM/misc/v1.0/opt-125m",
    # microsoft/Phi-3.5-vision-instruct、Qwen/Qwen3-0.6B：仅被 -k 排除的
    # test_chat_multi_image / test_chat_extra_kwargs 的函数级 fixture 引用，
    # 不会实例化，不建 symlink
}
...
root = ET.Element("testsuites", name="vLLM PPU Entrypoints LLM (GHA)")
for label in LABELS:
...
lines = ["### Entrypoints LLM Test (PPU)", "",
...
if [ "${MODE}" = "single" ]; then
  # Aone single 是 1-PPU pod：限 1 卡使 tp=2 cases 自动 skip，语义对齐

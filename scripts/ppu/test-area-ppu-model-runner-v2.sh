...
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/model_runner_v2.yaml 的
# 迁移快照（见下方 MRV2_*_ARGS，调整用例直接改这里）。
# 模型走 /nas_aisw 预置卷（docker -v /nas_aisw:/nas_aisw + HF_HUB_CACHE）。
#
# 环境变量：
#   TEST_MODE   all(默认) | single | multi   — 对应 Aone 两个 ptg-ai-test job
#
# 机制移植自 aone_ci/scripts/test_area_ppu_model_runner_v2.sh（该文件
# AUTO-GENERATED 不可手改，故在此复刻）：
#   - single: 6 个独立 step（test_llm_engine / async_scheduling / context_length /
#     min_tokens / struct_output / spec_decode_e2e），单进程单卡
#   - multi:  4 个独立 step（basic_correctness distributed / async_llm_dp /
#     eagle_dp / pipeline_parallel），测试内部用 2 卡（TP/DP/PP=2）

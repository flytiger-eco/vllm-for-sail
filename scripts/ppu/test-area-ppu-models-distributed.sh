#!/bin/bash
# ==============================================================================
# scripts/ppu/test-area-ppu-models-distributed.sh — PPU Models Distributed 测试执行（GitHub Actions）
# ------------------------------------------------------------------------------
# 调用方：.github/workflows/test-area-ppu-models-distributed.yml（容器内，cwd = /workspace）。
#
# 完全自包含，不依赖 aone_ci/。用例选集是 aone_ci/ppu_extras/models_distributed.yaml 的
# 迁移快照（见下方 MD_MULTI_* 数组，调整用例直接改这里）。
# 模型走 /nas_aisw 预置卷（docker -v /nas_aisw:/nas_aisw + HF_HUB_CACHE）。
#
# 环境变量：
#   TEST_MODE   all(默认) | single | multi   — 对应 Aone 两个 ptg-ai-test job
#
# 机制移植自 aone_ci/scripts/test_area_ppu_models_distributed.sh（该文件
# AUTO-GENERATED 不可手改，故在此复刻）：
#   - single: 无 step（Aone ppu:1 pod 对本 area 无 single 用例，直接返回）
#   - multi:  3 个单进程 step（basic_correctness_distributed / sharded_state_loader
#     / transformers_distributed），每个测试内部 TP=2 / num_gpus=2 自行用 2 卡
...
TMP_JUNIT="/tmp/ppu-models-distributed-junit"
mkdir -p "${RESULTS_DIR}" "${TMP_JUNIT}"

# [deps] 无 area 特有 pip 依赖：基础测试工具链（pytest-asyncio/tblib/
# pytest-shard）由 ppu_install_dependency.sh 统一安装；ray backend 用例已用
# -k "not ray" 排除（PPU CI 无 ray），故不装 ray。

# ------------------------------------------------------------------------------
# [tests] 用例选集（快照自 aone_ci/ppu_extras/models_distributed.yaml multi 段）
# ------------------------------------------------------------------------------
# single = Aone ppu:1 pod：本 area 无 single 用例（extras `#single: []`），
#   single/all 模式均只跑下方 3 个 multi step。
#
# 上游 .buildkite/test_areas/models_distributed.yaml 1 step / 6 command，PPU
# 仅保留 Cmd 1-3（均 2 卡 TP=2 / num_gpus=2）；Cmd 4/5/6 跳过（原 yaml 注释）：
#   Cmd 4 models/language distributed — 超时（与 Cmd1-3 合计 >7200s），首版 defer
#   Cmd 5 models/multimodal distributed — 3x7B 最重 + test_common VIDEO_ASSETS 下载
#   Cmd 6 test_whisper distributed — whisper-large-v3-turbo 未 aliased + audio
# 文件级 ignore：test_maverick.py（Maverick-17B-FP8 未 aliased）——本快照未纳入
#   test_maverick，天然不在选集内。

# Cmd 1: basic_correctness distributed（TP=2）；-k "not ray"（PPU CI 无 ray）。
# L4 suite 选中 opt-125m(mp) + Llama-3.2-1B-Instruct(mp)；A100 组 test_suite
# 过滤自动 skip（TARGET_TEST_SUITE=L4，见 [env]）。
MD_MULTI_BASIC_CORRECTNESS_ARGS=(
...
# Cmd 2: sharded_state_loader（TP=1,2）；-k "not enable_lora"（adapter 未 staged）。
# fixture 内 snapshot_download(Llama-3.2-1B-Instruct)，已 aliased + HF cache
# symlink 命中，离线可解析。
MD_MULTI_SHARDED_STATE_LOADER_ARGS=(
  tests/model_executor/model_loader/test_sharded_state_loader.py
  -k
  "not enable_lora"
)

# Cmd 3: test_transformers distributed（TP=2）；model_impl=transformers，
# model: Llama-3.2-1B-Instruct（aliased）。
MD_MULTI_TRANSFORMERS_ARGS=(
...
# 快照自 extras env（对位上游 `TARGET_TEST_SUITE=L4 pytest basic_correctness/`）：
# test_models_distributed 用它过滤 model×backend 组合，L4 组选 opt-125m +
# Llama-3.2-1B-Instruct，A100 组自动 skip。test 内默认值本就是 L4，此处显式对齐。
export TARGET_TEST_SUITE="${TARGET_TEST_SUITE:-L4}"
...
MODEL_MAP = {
    # Cmd 1 basic_correctness distributed（opt-125m mp）+ 上游多处引用。
    # 清单命中：aone alias facebook/opt-125m=/ppusw/.../misc/v1.0/opt-125m
    # （→ /nas_aisw；与已落地 basic-correctness area 取值一致，已验证）。
    "facebook/opt-125m": "/nas_aisw/datasets/checkpoints/LLM/misc/v1.0/opt-125m",
    # Cmd 1/2/3 共用主模型（basic_correctness mp / sharded_state_loader
    # snapshot_download / test_transformers TP=2）。
    # 清单命中：checkpoints_cleaned.json path=checkpoints/LLM/Llama/v3.2/
    # Llama-3.2-1B-Instruct（清单 ms_name 为 unsloth/...，按 name 匹配；
    # Aone /ppusw 侧是 v3.1 目录，以红区清单的 v3.2 为准）。
...
root = ET.Element("testsuites", name="vLLM PPU Models Distributed (GHA)")
for label in LABELS:
...
lines = ["### Models Distributed Test (PPU)", "",
...
# multi 段 3 个 step（复刻 aone sh multi 分支的调用序）：均单进程，测试内部
# TP=2 / @multi_gpu_test(num_gpus=2) 自行用 2 卡，故不设 CUDA_VISIBLE_DEVICES，
# 让 pod 注入的 0/1 两卡全部可见。
_run_multi_steps() {
  _run_step "basic_correctness_distributed" 1 "${MD_MULTI_BASIC_CORRECTNESS_ARGS[@]}"
  _run_step "sharded_state_loader" 1 "${MD_MULTI_SHARDED_STATE_LOADER_ARGS[@]}"
  _run_step "transformers_distributed" 1 "${MD_MULTI_TRANSFORMERS_ARGS[@]}"
}

if [ "${MODE}" = "single" ]; then
  # 本 area 无 single 用例（Aone ppu:1 pod 对 models_distributed 无 single step）
  echo "[run] no single steps configured for this area"
